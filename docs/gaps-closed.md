# Async::DBD::Pg — Closed Gaps

Resolved issues, kept for the reasoning rather than the status. Several record
a conclusion that was later reversed by measurement — item 71 in particular
concluded from careful source reading that the pub/sub listener pause was
"probably not load-bearing", and the experiment it called for showed that
removing it deadlocks the first control query on both event loops.

Open items are in `gaps.md`.
